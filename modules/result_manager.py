...
def create_session_folder(
    ...
    Other sports:
        results/Sprinting/Sprinting/timestamp/
    """
    ...
    if sport == "Weightlifting" and camera_view != "N/A":
        camera_view_folder = clean_name(camera_view)
        session_path = (
            Path("results")
            / sport_folder
            / exercise_folder
...
def generate_dummy_outputs(session_path: Path, sport: str, exercise: str):
    ...
    Kept for testing older buttons if needed.
    Real V7/V8 analysis now uses AnalysisRecorder.
    """
    ...
    df.to_csv(csv_path / "dummy_joint_angles_and_velocity.csv", index=False)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(plots_path / "dummy_joint_angles.png", dpi=300)
    plt.close()